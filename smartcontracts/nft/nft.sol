...
contract NFTPass is NFTokenEnumerable,  Ownable, PC { 
  	...
  	mapping (uint256 => address) public administeredBy;
    ...
    * all minted tokens could become unusable 
    */
    ...
    */
    function setNFTSymbol(string calldata newSymbol) external onlyOwner{
  	...
  	/**
    * @dev key function to create passes, 1 per wallet 
    */
  ...
  function _getSVG() internal pure returns (string memory){
      return string(abi.encodePacked("<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' viewBox='0 0 300 300' height='300' width='300'><rect x='0' y='0' rx='10' ry='10' width='300' height='300' style='fill:black;stroke:" 
                              , _getBorderColor(),";stroke-width:20;opacity: 0.85' /><text x='50%' y='25%' text-anchor='middle' fill='white' font-size='14' font-family='Verdana'>Certificate</text><text x='50%' y='45%' text-anchor='middle' fill='white' font-size='14' font-family='Verdana' >NFT Pass</text></svg>" 
                              ));
  }
  
  function _getDescription() internal pure returns (string memory){
      return 'NFT Pass, used to authorize access';
  }
  ...
  /**
    * @dev user can burn own pass, why not..(admin can burn pass of any user)
    **/
  function burn(uint256 _tokenId) public {
    require(msg.sender == owner || idToOwner[_tokenId] == msg.sender || administeredBy[_tokenId] == msg.sender, "no rights to burn");  
    super._burn(_tokenId);
  }

  function burnByAddress(address user) external{
  	uint256 _tokenId = tokenOfOwnerByIndex(user, 0);
    burn(_tokenId);
  }
